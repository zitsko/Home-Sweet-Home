...
router.put("/:id", async (req, res) => {
    try {
        const updateHome = await HomeModel.findByIdAndUpdate(req.params.id,
            { $set: req.body },
            { new: true });//set method is mongodB method the inside in the set we pass what we are going to change, the new will return the new version of our document
